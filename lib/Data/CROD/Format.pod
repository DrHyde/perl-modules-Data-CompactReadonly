...
Data::CROD::Format - a description of CROD data format, version 0.

=head1 NOTATION

Bytes with values that are printable ASCII will be shown as a single ASCII character.
Otherwise bytes will be shown either in hexadecimal C<0xAB> or binary X<0b01010101>.

When appropriate bit-fields will be shown in binary C<0b100>.

Bytes will be separated by spaces, bit-fields by hyphens.
...
Encodes a list of key-value pairs, the keys being strings and the values being
nodes of any type. The keys B<must> be stored in ASCIIbetical order.
...
telling us how much data is in the node. The type specifier is a bit field. The
first five bits tell us the data type. The first two bits will tell us whether
the node is a collection or not.
...
some of them tell us what type is used to encode the collection's length:
    ...
    0b1000 - Huge (valid as a length)        0b1001 - NegativeHuge
...
"北京市" is stored as the 9 bytes:
    ...
    北: E5 8C 97    京: E4 BA AC    市: E5 B8 82
...
database header. The first pointer in each pair must point to a Text node
which will be used as a key for looking up values. The second pointer in each
pair points to the value, which can be any type of node. The pointers to keys
must list them in ASCIIbetical order.
