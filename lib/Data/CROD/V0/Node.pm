...
sub _create {
    my($class, %args) = @_;
    die("fell through to Data::CROD::V0::Node::_create when creating a $class\n")
           ...
           !ref($data)
             ? 'Text::'.do {
                            $class->_sub_type_for_collection_of_length(
                                length(Data::CROD::V0::Text->_text_to_bytes($data))
...
sub _type_class {
    ...
    my $type_name = "Data::CROD::V0::".$class->$map_method($in_type);
    eval "use $type_name";
    die($@) if($@);
    return $type_name;
}
...
# caught in Data::CROD::V0->create(), the pointer size incremented, and it will
