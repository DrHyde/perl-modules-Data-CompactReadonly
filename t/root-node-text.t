...
is(Data::CROD->read($fh), 'hippo', "can read a Text with Byte length");

open($fh, '<', \"$header_bytes${b00001000}\x00\x05hippo");
is(Data::CROD->read($fh), 'hippo', "can read a Text with Short length");

open($fh, '<', \"$header_bytes${b00010000}\x00\x00\x05hippo");
is(Data::CROD->read($fh), 'hippo', "can read a Text with Medium (24 bits) length");
...
is(Data::CROD->read($fh), "\x{5317}\x{4eac}\x{5e02}", "bytes are converted to utf-8 text: got 3 chars [北, 京, 市] from 9 bytes");
