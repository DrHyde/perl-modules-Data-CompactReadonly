...
is(my $data = Data::CROD->read($filename), undef, "can create a Null file");
...
foreach my $tuple (
    ...
    foreach my $value ($value, -$value) {
        ...
        is($data = Data::CROD->read($filename), $value,
...
foreach my $value (5.1413, 81.72e-50, -1.37e100/3) {
    Data::CROD->create($filename, $value);
    is($data = Data::CROD->read($filename), $value, "can create a Float file ($value)");
}
...
foreach my $length (1, 1000, 100000, 0x1000000) {
    ...
    my $value = 'x' x $length;
    Data::CROD->create($filename, $value);
    is($data = Data::CROD->read($filename), $value, "can create an ASCII Text file ($length chars)");
...
foreach my $length (1, 1000) {
    ...
    Data::CROD->create($filename, $value);
    is($data = Data::CROD->read($filename), $value, "can create a non-ASCII Text file ($length times three chars, each 3 utf-8 bytes)");
        is((stat($filename))[7], $filesize, "... file is expected size $filesize");
}
...
foreach my $tuple ( # torture tests
    ...
    my($value, $filesize) = @{$tuple};
    Data::CROD->create($filename, $value);
    is($data = Data::CROD->read($filename), $value, "can create a file with value '$value'");
    is((stat($filename))[7], $filesize, "... file is expected size for data $value") || diag(`hexdump -C $filename`);
}
